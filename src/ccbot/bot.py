...
async def post_init(application: Application) -> None:
    ...
    # Register on every scope Zell chats in.  Clients read the scope that matches
    # the chat, so forum topics need all_group_chats set explicitly — falling
    # back to default is unreliable for the composer's "/" button.
    for scope in (
        None,
        BotCommandScopeAllPrivateChats(),
        BotCommandScopeAllGroupChats(),
